/**
 * Mirrors the `class_materials` table schema for read-only public access.
 * Replace MOCK_CLASS_MATERIALS with a DB query when the backend is wired.
 */
...
/** Opens PDFs/images in the browser; routes Office files through Microsoft's online viewer. */
export function getMaterialPreviewUrl(fileUrl: string, fileName: string): string {
...
export function getMaterialsByClass(
  classNumber: ClassNumber,
  materials: ClassMaterial[] = MOCK_CLASS_MATERIALS,
): ClassMaterial[] {
  return materials
    .filter((m) => m.class_number === classNumber)
